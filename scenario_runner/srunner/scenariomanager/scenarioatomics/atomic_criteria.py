...
class CollisionTest(Criterion):
    ...
    def _compute_pedestrian_mais3p(self,ego_velocity, ped_velocity):
        """
        Predict MAIS3+ injury risk for a pedestrian struck by a vehicle,
        using the Yanagisawa et al. logistic model based on real-world crash data.

        Parameters:
        - ego_velocity: carla.Vector3D of ego vehicle velocity
        - ped_velocity: carla.Vector3D of pedestrian velocity

        Returns:
        - mais3p_risk: float (probability between 0 and 1)
        - impact_speed_kph: float (relative impact speed in km/h)
        - relative_speed_mps: float (relative speed in m/s)
        """
        # Convert CARLA vectors to NumPy
        v_ego = np.array([ego_velocity.x, ego_velocity.y, ego_velocity.z])
        v_ped = np.array([ped_velocity.x, ped_velocity.y, ped_velocity.z])

        # Compute relative velocity vector and magnitude (in m/s)
        rel_v = v_ego - v_ped
        rel_speed = np.linalg.norm(rel_v)

        # Yanagisawa logistic regression model
        # Source: PCDS model adapted to GES (Equation 3.16)
        beta0 = 3.164
        beta1 = 0.288
        logit = beta0 - beta1 * rel_speed

        probability = 1 / (1 + np.exp(logit))
        return probability, np.linalg.norm(v_ego) * 3.6, rel_speed
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        if ('static' in event.other_actor.type_id or 'traffic' in event.other_actor.type_id) \
            ...
            # Compute MAIS3+ injury risk
            ego_velocity = self.actor.get_velocity()
            ped_velocity = event.other_actor.get_velocity()
            mais3p_risk, impact_speed_kph, pdof_deg = self._compute_pedestrian_mais3p(ego_velocity, ped_velocity)

            self.logger.info(
                f"🚨 MAIS3+ risk: {mais3p_risk:.1%} | Speed: {impact_speed_kph:.1f} km/h | PDOF: {pdof_deg:.1f}°"
            )
            print(f"🚨 MAIS3+ risk: {mais3p_risk:.1%} | Speed: {impact_speed_kph:.1f} km/h | PDOF: {pdof_deg:.1f}°")
            ...
            collision_event.set_message(
                f"MAIS3+ risk: {mais3p_risk:.1%} | Pedestrian collision with actor id={event.other_actor.id} "
                f"at (x={round(actor_location.x, 3)}, y={round(actor_location.y, 3)}, z={round(actor_location.z, 3)})"
            )
