...
class LeaderboardEvaluator(object):
    ...
    def _cleanup(self):
        ...
        for i in self.world.get_actors():
            if i.type_id.startswith('vehicle') or i.type_id.startswith('walker'):
                i.destroy()
...
def main():
    ...
    parser.add_argument('--traffic-manager-port', default=8030, type=int,
    ...
    parser.add_argument("--config", type=str, default='',
                        help="Path to HABIT config.yaml (default: auto-detect)")

    arguments = parser.parse_args()

    # Set config path for downstream modules (e.g., route_scenario.py)
    if arguments.config:
        os.environ["HABIT_CONFIG"] = os.path.abspath(arguments.config)
    ...
    print('✅ Python finished normally.')
    if crashed:
        sys.exit(-1)
    else:
        sys.exit(0)
