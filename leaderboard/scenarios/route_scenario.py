...
from srunner.scenarios.ped_backgound_activity import PedBackgroundBehavior
...
from leaderboard.utils.config_loader import load_config
...
class RouteScenario(BasicScenario):
    ...
    def _spawn_ego_vehicle(self):
        ...
        ego_vehicle = CarlaDataProvider.request_new_actor('vehicle.mercedes.coupe_2020',
                                                          elevate_transform,
                                                          rolename='hero')
        if not ego_vehicle:
            return
    ...
    def _create_behavior(self):
        ...
        config = load_config()
        motions_dir = config["data"]["motions_dir"]
        spawn_file = config["data"]["spawn_points"]
        csvs_dir = config["data"]["csvs_dir"]

        not_crossing = pd.read_csv(os.path.join(csvs_dir, 'Not_Crossing.csv'))
